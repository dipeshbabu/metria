"""SGLang backend for KV Fidelity.
...
right server with the right KV cache config before running KV Fidelity.
...
``KV_FIDELITY_SGLANG_REF_URL`` and ``KV_FIDELITY_SGLANG_CAND_URL``. For other
methods only ``KV_FIDELITY_SGLANG_URL`` is needed.

Env knobs
---------

  KV_FIDELITY_SGLANG_URL       single-server endpoint (default
                           http://127.0.0.1:30000)
  KV_FIDELITY_SGLANG_REF_URL   reference-config endpoint for run_kld
  KV_FIDELITY_SGLANG_CAND_URL  candidate-config endpoint for run_kld
  KV_FIDELITY_SGLANG_TIMEOUT   per-request timeout in seconds (default 600)
  KV_FIDELITY_SGLANG_KLD_TOPK  top-K for prompt logprobs during KLD
                           (default 64)
...
def _timeout() -> float:
    return float(os.environ.get("KV_FIDELITY_SGLANG_TIMEOUT", "600"))
...
def _post(url: str, path: str, body: dict, *, timeout_s: float) -> dict:
    ...
    try:
        ...
        raise BackendCapabilityError(
            ...
            f"Set KV_FIDELITY_SGLANG_URL or launch a server."
        ) from e
...
def _load_tokenizer(model_id: str):
    ...
    try:
        ...
        raise BackendCapabilityError(
            ...
            "sglang extra and ensure the model/tokenizer is locally "
...
class SGLangBackend(Backend):
    ...
    def run_completion(
        ...
        *,
        ...
        url = _url("KV_FIDELITY_SGLANG_URL")
    ...
    def run_kld(
        ...
        *,
        ...
        ref_url = os.environ.get("KV_FIDELITY_SGLANG_REF_URL")
        cand_url = os.environ.get("KV_FIDELITY_SGLANG_CAND_URL")
        if not ref_url or not cand_url:
            raise BackendCapabilityError(
                "run_kld on SGLang requires two servers running concurrently "
                "with different KV configs. Set KV_FIDELITY_SGLANG_REF_URL and "
                "KV_FIDELITY_SGLANG_CAND_URL, or use --backend vllm / --backend "
                "llamacpp which can hot-swap KV configs in-process."
            )
        ref_url = ref_url.rstrip("/")
        cand_url = cand_url.rstrip("/")
        topk = int(os.environ.get("KV_FIDELITY_SGLANG_KLD_TOPK", "64"))

...
patched ``llama-completion`` (KV_FIDELITY_TRAJECTORY env var → JSONL stream of
...
This module's signature mirrors :mod:`kv_fidelity.axes.gtm.run_gtm` so callers
can swap the axis with a one-line import change. The composite-score code
in :mod:`kv_fidelity.score` is unchanged: it accepts any 0–100 score for
"axis A".
    ...
    for i, p in enumerate(prompts):
        ...
        if not ref_toks and not cand_toks:
            raise RuntimeError(
                ...
                f"tools/completion/completion.cpp (KV Fidelity v0.1.4 patch)."
            )

"""Tests for kv_fidelity.backends: registry, base ABC defaults, llamacpp adapter,
...
from kv_fidelity.backends import auto_backend, get_backend
...
from kv_fidelity.backends.sglang import SGLangBackend, _validate_kv_str
from kv_fidelity.backends.vllm import VLLMBackend, _kv_str_to_vllm_dtype
...
def test_auto_backend_gguf_picks_llamacpp(tmp_path, monkeypatch):
    monkeypatch.delenv("KV_FIDELITY_BACKEND", raising=False)
...
def test_auto_backend_env_override_takes_precedence(tmp_path, monkeypatch):
    monkeypatch.setenv("KV_FIDELITY_BACKEND", "vllm")
...
def _run_fake_topk_kld(
    ...
    from kv_fidelity.backends import sglang as module

    reference_url = "http://reference.test"
    candidate_url = "http://candidate.test"
    monkeypatch.setenv("KV_FIDELITY_SGLANG_REF_URL", reference_url)
    monkeypatch.setenv("KV_FIDELITY_SGLANG_CAND_URL", candidate_url)
...
def test_sglang_run_kld_requires_dual_urls(monkeypatch):
    ...
    monkeypatch.delenv("KV_FIDELITY_SGLANG_REF_URL", raising=False)
    monkeypatch.delenv("KV_FIDELITY_SGLANG_CAND_URL", raising=False)
    bk = SGLangBackend()
    with pytest.raises(BackendCapabilityError) as exc:
        bk.run_kld(
            model=Path("/tmp/x"),
    ...
    assert "KV_FIDELITY_SGLANG_REF_URL" in str(exc.value)
...
def test_llamacpp_run_completion_delegates(monkeypatch):
    ...
    monkeypatch.setattr("kv_fidelity.runner.run_completion", fake_rc)
...
def test_llamacpp_run_completion_trajectory_delegates(monkeypatch):
    ...
    monkeypatch.setattr("kv_fidelity.runner.run_completion_trajectory", fake_rct)
...
def test_llamacpp_tokenize_delegates(monkeypatch):
    bk = LlamaCppBackend()
    monkeypatch.setattr(
        "kv_fidelity.runner.tokenize_to_ids",
...
def test_mlx_translate_kv_symmetric_q8():
    from kv_fidelity.backends.mlx import _translate_kv_to_mlx
...
def test_mlx_require_mlx_raises_if_missing(monkeypatch):
    ...
    from kv_fidelity.backends import mlx as mlx_mod
